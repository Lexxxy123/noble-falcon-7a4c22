...
case "resetcount":
  ...
  break;
      ...
      case "refreshapi":
        await interaction.reply({ content: "Refreshing API key...", ephemeral: true });
        try {
          await getnewkey(queryParams)
          let response = `Sent request! Check back soon.`;
          await interaction.editReply({ content: response });
        } catch (error) {
          console.error("Error refreshing API key:", error);
          await interaction.editReply({ content: "An error occurred while refreshing API key." });
        }
        break;
      ...
      case "showdatabase":
        ...
        try {
          ...
          for (const table of tables) {
            const columns = await queryParams(`PRAGMA table_info(${table.name})`, []);
